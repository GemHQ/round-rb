...
module Round
  ...
  def self.client(url=nil)
    @client ||= begin
      url ||= Round.url
      pb = Round::Patchboard.discover(url) { Round::Patchboard::Context.new }
      pb.spawn
    end
  end

  def self.authenticate(options)
    url = options[:url] || Round.url
  ...
  class Patchboard < Patchboard
    ...
    def self.client
      @@patchboard ||= self.discover(BASE_URL, :namespace => self::Resources) { Round::Patchboard::Context.new }
    ...
    class Client < Patchboard::Client
      ...
      def application
        unless @application
          ...
          @application = Round::Application.new(resource: application_resource)
        end
      ...
      def users
        unless @users
          ...
          @users = Round::UserCollection.new(resource: users_resource)
        end
      ...
      def wallet(options = {})
        ...
        Round::Wallet.new(resource: wallet_resource)
      end
    end
